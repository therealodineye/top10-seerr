...
def fetch_html_flixpatrol(url):
    """
    Fetches HTML from FlixPatrol. First attempts direct request with curl_cffi.
    If blocked by Cloudflare (403 or Turnstile challenge), falls back to FlareSolverr.
    """
    try:
        res = requests.get(url, impersonate="chrome", timeout=10)
        if res.status_code == 200 and "Just a moment..." not in res.text:
            return res.text
        print(f"[SCRAPER WARNING] Direct fetch got HTTP {res.status_code}. Attempting FlareSolverr fallback...")
    except Exception as e:
        print(f"[SCRAPER WARNING] Direct fetch exception ({e}). Attempting FlareSolverr fallback...")

    # Fallback to FlareSolverr
    flaresolverr_urls = [
        os.getenv("FLARESOLVERR_URL", "http://flaresolverr:8191/v1"),
        "http://10.20.1.114:8191/v1",
    ...
    for fs_url in flaresolverr_urls:
        try:
            ...
            resp = std_requests.post(fs_url, json=payload, timeout=35)
            data = resp.json()
            if data.get("status") == "ok":
                html = data.get("solution", {}).get("response", "")
                if html and "Just a moment..." not in html:
                    return html
                print("[SCRAPER WARNING] FlareSolverr response contained Turnstile challenge page.")
            else:
                print(f"[SCRAPER WARNING] FlareSolverr ({fs_url}) returned status: {data.get('status')}")
        except Exception as e:
            print(f"[SCRAPER WARNING] FlareSolverr ({fs_url}) request failed: {e}")
...
def get_titles_flixpatrol(platform, country_slug):
    ...
    for i in range(6): 
        ...
        # Inject defensive 3-second delay between separate network attempts
        if i > 0:
            print("[SCRAPER] Defensively waiting 3 seconds before fallback attempt...")
            time.sleep(3)
            
        try:
            html = fetch_html_flixpatrol(url)
            if html:
                ...
                print(f"[SCRAPER WARNING] Failed to retrieve valid HTML on {date_str}")
        except Exception as e:
